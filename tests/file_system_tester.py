...
class FileSystemTester(object):
	...
	def upload_files(self):
		...
		files = [self.config.file_01, self.config.file_02, self.config.file_03, self.config.file_04, self.config.file_05]
		for file in files:
	...
	def delete_dir(self, dir: FileAPI):
		self.__logger.info(f"> Deleting {dir}")
		dir.delete()
		if not dir.exists():
			self.__logger.info(f"> Deleted {dir}")
		else:
			self.errors.append(f"> Failed to delete {dir}")
			self.__logger.error(f"> Failed to delete {dir}")
	...
	def list_dir(self, dir: FileAPI):
		self.__logger.info(f"> Listing dir {dir}")
		children = dir.list_children()
		if len(children) == 0:
			self.errors.append(f"> Failed to list children of dir {dir}")
			self.__logger.error(f"> Failed to list children of dir {dir}")
		else:
			for file in dir.list_children():
				self.__logger.info(f"> Found file {file}")
